...
static void scr_timer_draw_units() {
	...
	if (timer_mode & MODE_UNIT_MIN) {
		mlcd_draw_text(I18N_TRANSLATE(MESSAGE_HOURS), 0, TIME_Y_POS+55, MLCD_XRES/2, 0, FONT_OPTION_NORMAL, HORIZONTAL_ALIGN_CENTER);
		mlcd_draw_text(I18N_TRANSLATE(MESSAGE_MINUTES), MLCD_XRES/2, TIME_Y_POS+55, MLCD_XRES/2, 0, FONT_OPTION_NORMAL, HORIZONTAL_ALIGN_CENTER);
	} else {
		mlcd_draw_text(I18N_TRANSLATE(MESSAGE_MINUTES), 0, TIME_Y_POS+55, MLCD_XRES/2, 0, FONT_OPTION_NORMAL, HORIZONTAL_ALIGN_CENTER);
		mlcd_draw_text(I18N_TRANSLATE(MESSAGE_SECONDS), MLCD_XRES/2, TIME_Y_POS+55, MLCD_XRES/2, 0, FONT_OPTION_NORMAL, HORIZONTAL_ALIGN_CENTER);
	}
}
...
static void scr_draw_saved_timers() {
	...
	for (int i = 0; i < 4; i++) {
		...
		txt[3] = '0'+t2/10;
		txt[4] = '0'+t2%10;
		mlcd_draw_text(txt, (i&1)*72, TIME_Y_POS+80+20*(i>>1), MLCD_XRES/2, 0, FONT_OPTION_NORMAL, HORIZONTAL_ALIGN_CENTER);
	}
	scr_mark_timer();
}
...
static void scr_draw_timer_all() {
	if ((timer_mode & MODE_VISIBLE) == 0)
		return;
  mlcd_draw_text(I18N_TRANSLATE(MESSAGE_TIMER), 10, 5, 80, 0, FONT_OPTION_BIG, HORIZONTAL_ALIGN_LEFT);
